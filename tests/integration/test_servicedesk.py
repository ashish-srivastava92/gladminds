...
class TestServiceDeskFlow(BaseTestCase):
    ...
    def test_get_feedback_sdo(self):
        system = self.system
        system.post_feedback()
        system.login(username='sdo', password='123', provider='desk', group_name='SDO')
        system.get_feedback_information()
        feedback_obj = aftersell_common.Feedback.objects.all()
        self.assertEqual(feedback_obj[0].priority, "High")
    ...
    def test_get_feedback_sdm(self):
        system = self.system
        system.post_feedback()
        system.login(username='sdm', password='123', provider='desk', group_name='SDM')
        system.get_feedback_information()
        feedback_obj = aftersell_common.Feedback.objects.all()
        self.assertEqual(feedback_obj[0].priority, "High")
    ...
    def test_sms_email_assignee_after_feedback_assigned(self):
        system = self.system
        system.post_feedback()
        system.login(username='sdm', password='123', provider='desk', group_name='SDO')
        system.update_feedback(status='Open')
        log_len_after = logs.AuditLog.objects.all()
        self.assertEqual(log_len_after[0].reciever, "9999999998")
    ...
    def test_sms_email_after_resolved(self):
        ...
        system.post_feedback()
        system.login(username='sdo', password='123', provider='desk', group_name='SDO')
        system.update_feedback(status='resolved', assign_to='+919999999999')
 
    def test_updated_feedback(self):
        system = self.system
        system.post_feedback()
        system.login(username='sdm', password='123', provider='desk', group_name='SDM')
        system.update_feedback(status='Closed', assign_to='None')
        feedbacks = aftersell_common.Feedback.objects.filter(priority='High')
        system.assertEqual(feedbacks[0].status ,'Closed')
