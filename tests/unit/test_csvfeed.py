...
class CSVFeedTest(GladmindsUnitTestCase):
    ...
    def test_brandproducttypefeed(self):
        ...
        obj.import_data()
        producttype_data = models.ProductType.objects.get(product_type = 'PRODUCTTYPE001_'+tmp)
    ...
    def test_import_duplicate_feed(self):
        ...
        obj.import_data()
        producttype_data = models.ProductType.objects.get(product_type = 'PRODUCTTYPE003_'+tmp)
        self.assertEqual(producttype_data.product_type, 'PRODUCTTYPE003_'+tmp, 'Product type not matched')
          
        #Check Duplicate
        producttype_data = models.ProductType.objects.filter(product_type = 'PRODUCTTYPE002_'+tmp)
...
class CSVFeedByFile(GladmindsUnitTestCase):
    ...
    def _test_brand_import(self):
        brand_data = models.Brand.objects.get(brand_id = 'HERO001')
        self.assertEqual(brand_data.brand_name, 'HERO')
 
    def _test_service_advisor(self):
        sa_data = models.ServiceAdvisor.objects.get(phone_number = '+SA0000000002')
        self.assertEqual(sa_data.dealer_id.dealer_id, 'TESTD001')
         
        sa_data = models.ServiceAdvisor.objects.get(phone_number = '+SA0000000003')
        self.assertEqual(sa_data.dealer_id.dealer_id, 'TESTD002')
 
    def _test_product_dispatch(self):
        product_data = models.ProductData.objects.get(vin = 'TESTDDZZZHER44300')
        ...
        coupon_count = models.CouponData.objects.filter(vin__vin = 'TESTDDZZZHER44300', status = 1).count()
        self.assertEqual(coupon_count, 2)
         
        coupon_count = models.CouponData.objects.filter(vin__vin = 'TESTDDZZZHER44301', status = 1).count()
    ...
    def _test_product_purchases(self):
        ...
        product_data = models.ProductData.objects.get(vin = 'TESTDDZZZHER44301')
    ...
    def _test_coupon_redeeem(self):
        coupon_data = models.CouponData.objects.get(vin__vin = 'TESTDDZZZHER44300', unique_service_coupon = 'HEROCOUP001')
#         self.assertEqual(coupon_data.sa_phone_number.phone_number, '+SA0000000001')
        self.assertEqual(int(coupon_data.actual_kms), 100)
         
        coupon_data = models.CouponData.objects.get(vin__vin = 'TESTDDZZZHER44301', unique_service_coupon = 'BAJAJCOUP002')
...
#         customer_data = models.GladMindUsers.objects.get(phone_number = '+CUST000000001')
