...
def get_customers_to_send_reminder(*args, **kwargs):
    ...
    results = common.CouponData.objects.filter(mark_expired_on__range=
...
def get_customers_to_send_reminder_by_admin(*args, **kwargs):
    ...
    results = common.CouponData.objects.filter(schedule_reminder_date__day=today.day,
...
def expire_service_coupon(*args, **kwargs):
    today = timezone.now()
    threat_coupons = CouponData.objects.filter(mark_expired_on__lte = today.date()).exclude(Q(status=2) | Q(status=3) | Q(status=6))
