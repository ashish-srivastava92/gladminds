...
def get_customers_to_send_reminder_by_admin(*args, **kwargs):
    from gladminds.tasks import send_reminder_message
    SCHEDULE_REMINDER_QUERY = """SELECT gc.id, gu.phone_number, unique_service_coupon, product_id, expired_date, valid_days, valid_kms FROM gladminds_customerdata gc inner join gladminds_gladmindusers gu on gc.phone_number_id = gu.id  WHERE is_closed !=1 AND is_expired!=1 AND schedule_reminder_date = DATE(NOW());"""
    usc_list=[]
    cursor = connection.cursor()
