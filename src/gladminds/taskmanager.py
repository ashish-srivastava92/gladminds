...
def expire_service_coupon(*args, **kwargs):
    UPDATE_EXPIRE_STATUS_FOR_SERVICE_COUPON = """UPDATE gladminds_coupondata SET status=3 where mark_expired_on<DATE(NOW())"""
    try:
        cursor = connection.cursor()
