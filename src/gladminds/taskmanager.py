...
def get_customers_to_send_reminder(*args, **kwargs):
    ...
    REMINDER_QUERY = """SELECT gc.id, ggu.phone_number, gc.unique_service_coupon, gu.vin, gc.mark_expired_on, gc.valid_days, gc.valid_kms FROM gladminds_coupondata gc inner join gladminds_productdata gu on gc.vin_id = gu.id inner join gladminds_gladmindusers ggu on ggu.id = gu.phone_number_id  WHERE DATE(mark_expired_on) = DATE_ADD(DATE(NOW()),INTERVAL {0} DAY) AND status !=2 AND status!=3 AND last_reminder_date is NULL;""".format(reminder_day)
    usc_list=[]
...
def get_customers_to_send_reminder_by_admin(*args, **kwargs):
    from gladminds.tasks import send_reminder_message
    SCHEDULE_REMINDER_QUERY = """SELECT gc.id, ggu.phone_number, gc.unique_service_coupon, gu.vin, gc.mark_expired_on, gc.valid_days, gc.valid_kms FROM gladminds_coupondata gc inner join gladminds_productdata gu on gc.vin_id = gu.id inner join gladminds_gladmindusers ggu on ggu.id = gu.phone_number_id  WHERE status !=2 AND status!=3 AND DATE(schedule_reminder_date) = DATE(NOW());"""
    usc_list=[]
    cursor = connection.cursor()
