...
def validate_coupon(sms_dict, phone_number):
    ...
    try:
        ...
        sms_log(brand=settings.BRAND, receiver=customer_phone_number, action=AUDIT_ACTION, message=customer_message)
        ...
        phone_number = utils.get_phone_number_format(phone_number)
        sms_log(brand=settings.BRAND, receiver=phone_number, action=AUDIT_ACTION, message=dealer_message)
...
def validate_service_advisor(phone_number, close_action=False):
    ...
    if message:
        ...
        sms_log(brand=settings.BRAND, receiver=sa_phone, action=AUDIT_ACTION, message=message)
