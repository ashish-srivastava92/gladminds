...
from gladminds.core.apis.authorization import MultiAuthorization, HistoryCustomAuthorization
...
class VisualisationUploadHistoryResource(CustomBaseModelResource):
    ...
    class Meta:
        ...
        authentication = AccessTokenAuthentication()
        authorization = MultiAuthorization(DjangoAuthorization(), HistoryCustomAuthorization())      
...
class BOMPlatePartResource(CustomBaseModelResource):
    ...
    def alter_list_data_to_serialize(self, request, data):
        ...
        elif request.GET.get('bom__sku_code'):
            sku_code= request.GET.get('bom__sku_code')
            print sku_code,"codeeeeeeeeeee"
            all_plates = get_model('BOMPlatePart').objects.filter(bom__sku_code=sku_code).values("plate__plate_id","plate__plate_txt").distinct()
            print all_plates,"alll"
            plates=[]
            
            for each_plate in all_plates:
                plate_ids={}
                print each_plate["plate__plate_id"]
                plate_ids["plate_id"]=each_plate["plate__plate_id"]
                plate_ids["plate_name"]=each_plate["plate__plate_txt"]
                plates.append(plate_ids)
            data['meta']['plates']=plates
            print data['meta']
        return data
    ...
    def dehydrate(self, bundle):
        ...
        print bundle.data['bom_visualization'], "aaaaaaaaaaaaaaaaaa"
        return bundle    
    ...
    def save_plate_part(self, request, **kwargs):
        ...
        try:
            ...
            upload_status = post_data.get('uploadStatus')
            dashboard_image = request.FILES['dashboardImage'] 
            sbom_part_mapping = []
           
            vis_obj = get_model('VisualisationUploadHistory').objects.filter(sku_code=sku_code, plate_id=plate_id, status="pending").exists()
            ...
            if vis_obj:
                if upload_status == "reject":   
                    reject_comment = post_data.get('comment') 
                    get_model('VisualisationUploadHistory').objects.filter(sku_code=sku_code, plate_id=plate_id, status="pending").update(status="reject", comment=reject_comment)
                    ...
                    print "saving trying", upload_history_data
                    upload_history_data.save()
                else:
                    
                    error_message = 'EXISTING'
                    return HttpResponse(json.dumps({"message" : error_message}), content_type="application/json", status=200)
            else:   
                upload_history_data = get_model('VisualisationUploadHistory')(sku_code=sku_code, bom_number=bom_number, plate_id=plate_id, eco_number=eco_number, status="Pending")
                print "saving trying", upload_history_data
                upload_history_data.save()


        
            for query in bom_queryset:
                temp = {}
            ...
            next(spamreader)  # ask dj if hes  sending with header or not
            ...
            for part_entry in csv_data:
                try:
                    ...
                    bompartplate_obj = bom_visual[0]['bomplatepart_object']
                    ...
                    if part_entry['x-axis'] and part_entry['y-axis'] and part_entry['z-axis']:
                        visual_obj = get_model('BOMVisualization')(history=upload_history_data,bom=bompartplate_obj,id=None,part_href = part_entry['href'],x_coordinate = int(part_entry['x-axis']),y_coordinate = int(part_entry['y-axis']),z_coordinate = int(part_entry['z-axis']),serial_number = part_entry['serial_number'])
...
class BOMVisualizationResource(CustomBaseModelResource):
    ...
    def preview_sbom_details(self, request, **kwargs):
        ...
        try:
            ...
            plate_images = "{0}/{1}".format('192.168.0.74:8004/static', plate_image)
            try:
                eco_i = get_model('ECOImplementation').objects.get(eco_number=eco_number)
                validation_date = eco_i.change_date
            except Exception as ex:
            ...
            plate_part_details = []
            new_plate_part=[]
            bom_visualisation = get_model('BOMVisualization').objects.filter(bom__in=bom_queryset,history=visualisation_data) 
            for data in bom_visualisation:             
                part_details = model_to_dict(data , exclude=['id', 'bom'])
                
                part_details["x_coordinate"] = [part_details["x_coordinate"]]
                part_details["y_coordinate"] = [part_details["y_coordinate"]]
                part_details["z_coordinate"] = [part_details["z_coordinate"]]
                ...
                part_details["revision_number"] = model_to_dict(data.bom.part , fields=['revision_number']).values()[0]
              
                for each in plate_part_details:
                    if (each['part_href'] == part_details['part_href']) and (each['part_number'] == part_details['part_number']):
                                x = part_details['x_coordinate'][0]
                                y = part_details['y_coordinate'][0]
                                z = part_details['z_coordinate'][0]
                                each['x_coordinate'].append(x)
                                each["y_coordinate"].append(y)
                                each["z_coordinate"].append(z)
                                 
                plate_part_details.append(part_details)
                if len(plate_part_details) == 0:
                    plate_part_details.append(part_details)
                new_list =[]
                new_list.append(plate_part_details[0])
                for each in plate_part_details:
                    for each_new in new_list:
                        print each["part_number"],"des" 
                        print each_new["part_number"] ,"des1"
                        if each["part_number"] != each_new["part_number"]:
                            flag=True          
                        else:
                            flag=False
                    if flag:
...
#             if not plate_image:
            ...
            output_data = {"plate_image":plate_images, 'plate_part_details': new_list, 'comments':comment_list}
            print output_data
    ...
    def review_sbom_details(self, request, **kwargs):
        ...
        try:
            ...
            bom_visualization.status = status  # no status is there
            if remarks:
