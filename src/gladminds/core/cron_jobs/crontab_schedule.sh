...
sudo crontab -l | (cat;echo "00 13 * * *  python /opt/app/gladminds/src/gladminds/core/cron_jobs/trigger_sqs_tasks.py export_customer_reg_to_sap gladminds-prod2") | sudo  crontab
sudo crontab -l | (cat;echo "00 19 * * *  python /opt/app/gladminds/src/gladminds/core/cron_jobs/trigger_sqs_tasks.py export_coupon_redeem_to_sap gladminds-prod2") | sudo  crontab
sudo crontab -l | (cat;echo "00 00 * * *  python /opt/app/gladminds/src/gladminds/core/cron_jobs/trigger_sqs_tasks.py send_report_mail_for_feed gladminds-prod2") | sudo crontab
