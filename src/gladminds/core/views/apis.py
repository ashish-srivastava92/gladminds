...
def pjp_schedule(request):
    ...
    for schedule in schedules:
        print schedule['dsr_id']
        dsr = DistributorSalesRep.objects.get(distributor_sales_code = schedule['dsr_id'])
        for location_details in schedule['pjp_location_details']:
            dsrworkallocation = DSRWorkAllocation()
            dsrworkallocation.dsr = dsr
            dsrworkallocation.distributor = dsr.distributor
            dsrworkallocation.pjp_created_date = schedule['pjp_creation_date']
            dsrworkallocation.locality = location_details['locality']
            dsrworkallocation.pjp_day = location_details['pjp_day']
            dsrworkallocation.save()
    
    return Response({'status':1, 'message':'Specified DSR schedule has been submitted'})
