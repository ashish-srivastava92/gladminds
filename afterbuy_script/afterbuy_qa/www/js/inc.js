...
var ipaddress = "https://api-qa.gladmindsplatform.co/gm/";
var s3address="https://s3.amazonaws.com/afterbuy/"
...
function testPost(form,cbfunction)
{
    ...
    postCORS(ipaddress, $(form).serialize(), cbfunction);
} 

function testGet(str,cbfunction){
	getCORS(ipaddress, str, cbfunction);
}
...
function populate_edit(data){
	...
	showLoading();
	getCORS(ipaddress+'?action=getProfile&unique_id='+localStorage.getItem("unique_id"), null, showEditSettings);
}
...
$(document).ready(function() {
	...
	$(".newPurchase").click(function(){
		...
		showLoading();
		getCORS(ipaddress+'?action=itemPurchaseInterest&item="'+interest_item+'"&unique_id='+localStorage.getItem("unique_id"), null, showConfirmation);
	...
	$(".waranteeExtend").click(function(){
		...
		showLoading();
		getCORS(ipaddress+'?action=waranteeExtend&item="'+warr_item+'"&unique_id='+localStorage.getItem("unique_id"), null, showConfirmation);
	...
	$(".insuranceExtend").click(function(){
		...
		showLoading();
		getCORS(ipaddress+'?action=insuranceExtend&item="'+ins_item+'"&unique_id='+localStorage.getItem("unique_id"), null, showConfirmation);
...
*/
	...
	$("#txtCountry").change(function(){
			showLoading();
			getCORS(ipaddress+'?action=getStates&cID='+$(this).val(), null, populateStates);
	});

	//Load states on change of country dropdown in Edit Profile.
	$("#txt_country").change(function(){
			showLoading();
			getCORS(ipaddress+'?action=getStates&cID='+$(this).val(), null, populateStates_edit);
	});
...
$('#dashboard').on('pagebeforeshow',function(event, ui)
{
	...
	$("#txtPassword").val("");
	$("#txtUsername").val("");
	$("#dashboard .profile").attr("src",s3address+localStorage.getItem("thumbURL"));
});
...
$('#profile-page').on('pagebeforeshow',function(event, ui)
{
	showLoading();
   	...
   	$("#profile-page .sp_username").html(localStorage.getItem("username"));
	$("#profile-page .sp_unique_id").html(localStorage.getItem("unique_id"));
	getCORS(ipaddress+'?action=getmyItems&unique_id='+localStorage.getItem("unique_id"), null, listMyItems);
...
function deleteItem(obj){
	...
	if(z){
		showLoading();
		getCORS(ipaddress+'?action=deleteRec&itemID='+$(obj).attr("rel"), null, function(data){
			//debugger;
			data = $.parseJSON(data);
			//alert(data.status)
			if(data.status==1){
...
function editItem(obj){
	...
	if(navigator.onLine){
		...
		getCORS(ipaddress+'?action=getProducts', null, function(data){
			...
			getCORS(ipaddress+'?action=fetchRec&itemID='+$(obj).attr("rel"), null, function(data){
				//debugger;
				data = $.parseJSON(data);
				//alert(data.status)
				if(data.status==1){
					...
					if(!invoice_URL=="")
						$("#edit-adding-item .add-invoice .epthumbnail img").attr("src",s3address+invoice_URL);

					if(!warranty_URL=="")
						$("#edit-adding-item .add-warranty .epthumbnail img").attr("src",s3address+warranty_URL);

					if(!insurance_URL=="")
						$("#edit-adding-item .add-insurance .epthumbnail img").attr("src",s3address+insurance_URL);
...
$('#item-warranty-check').on('pagebeforeshow',function(event, ui)
{
	...
	if(itemID == "a1" || itemID == "a2" || itemID == "a3" || itemID == "a4"){
		...
		getCORS(ipaddress+'?action=getWarantee&itemID='+localStorage.getItem("itemID"), null, warrantyCheck);
...
$('#item-insurance-check').on('pagebeforeshow',function(event, ui)
{
	...
	if(itemID == "a1" || itemID == "a2" || itemID == "a3" || itemID == "a4"){
		...
		getCORS(ipaddress+'?action=getInsurance&itemID='+localStorage.getItem("itemID"), null, insuranceCheck);
...
$('#myprofile').on('pagebeforeshow',function(event, ui)
{
	...
	$("#myprofile .profile").attr("src",s3address+localStorage.getItem("thumbURL"));
...
$('#user-settings').on('pagebeforeshow',function(event, ui){
	...
	showLoading();
	$("#editForm")[0].reset();
	$(".epthumbnail img").attr("src",s3address+localStorage.getItem("sourceURL"));
	$("#txt_country").val("india");

	getCORS(ipaddress+'?action=getStates&cID=india', null, populate_edit);
});
